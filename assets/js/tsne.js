...
// IMAGE ID SEARCH
function jumpToImageId() {
  var raw = document.getElementById('image-search').value.trim();

  if (!raw) return;

  var idx = -1;

  data.json.images.forEach(function(img, i) {
    if (idx !== -1) return;
  ...
  if (idx === -1) {
    alert('Image not found: ' + raw);
    console.log('First 20 image names:', data.json.images.slice(0, 20));
    return;
  }

  modal.close();
  world.flyToCellIdx(idx);

  console.log('Zoomed to:', data.json.images[idx], 'index:', idx);
}

// click search icon
document
  .getElementById('image-search-icon')
  .addEventListener('click', jumpToImageId);
